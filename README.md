...
# 管理台，一个组织有一个管理台服务
[admin]
192.168.112.152 ansible_ssh_user="user1" ansible_ssh_pass="Abc@123!" ansible_sudo_pass="Abc@123!"
...
## 动态水平添加单组织内部的 数据服务 或 计算服务 说明

上述操作均是一波启动了一系列的服务，但实际情况在系统运行中时，我们需要可以随时添加 数据服务 (存储能力) 和 计算服务 (算力) 的功能。 下面就是如何在运行时添加 数据服务 和 计算服务的说明，其中根据之前的部署情况我们区分为两种情况，【一】支持所有服务部署在一台机器上(即: 单个宿主机部署单个组织所有服务, 建议只在测试阶段使用); 【二】支持每台宿主机不是一个服务 (生产阶段建议使用这种)。


### 【一】支持所有服务部署在一台机器上时的 水平添加数据服务和计算服务 (即: 单个宿主机部署单个组织所有服务, 建议只在测试阶段使用)



### 【二】支持每台宿主机不是一个服务时的 水平添加数据服务和计算服务 (生产阶段建议使用这种)
