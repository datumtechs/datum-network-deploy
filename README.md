...
# 切换项目到 docker_v0.5.0 分支
git checkout docker_v0.5.0
...
# 拉取各服务的docker镜像并压缩保存到本地目录(mysql，admin，carrier，fighter,ice_via_consul)。
...
local_prepare.yml: 主控机执行环境检查，下载安装docker和docker-compose工具；拉取各服务的docker镜像并压缩保存到本地目录，建议不要乱动。

bootstrap.yml: 初始化集群各个节点，并下载安装docker和docker-compose等服务运行所需环境的剧本，建议不要乱动。
...
#### 6. admin nginx相关配置 (需要用户自行修改)

nginx_listen_port：基于TCP协议的nginx服务的监听端口，默认为80；配置此项主要是为了兼容宿主机的nginx服务；

nginx_listen_port_ssl: 基于SSL协议的nginx服务的监听端口，默认为443；配置此项主要是为了兼容宿主机的nginx服务；

#### 7. admin mysql 相关配置 (需要用户自行修改)

mysql_root_password: mysql root 账户密码。

admin_user: mysql 创建业务库的普通用户名称。

admin_password: mysql 普通用户的密码。

mysql_listen_port：mysql服务的监听端口，默认为3306；配置此项主要是为了兼容宿主机的mysql服务；

#### 8. 配置 admin web 端口号 (建议只开通内网端口策略)

admin_web_port: 为 admin 提供 web 服务的端口。

#### 9. carrier 服务的外网 ip 地址
...
```shell
# 在主控机上执行的准备工作，主要是下载安装docker和docker-compose工具，并拉取各服务的docker镜像并压缩保存到本地目录；在脚本 `datum-network-deploy` 根目录执行(需要输入 sudo 密码）：

ansible-playbook --ask-sudo-pass local_prepare.yml


# 初始化集群各个节点，主要是检查目标机的环境信息（操作系统版本，docker/docker-compose 安装），在脚本 `datum-network-deploy` 根目录执行：
...
```shell
# 在主控机上给各个目标机安装docker镜像文件和配置文件 (安装对应的服务)，在脚本 `datum-network-deploy` 根目录执行：
...
```shell
# 在主控机上启动各个目标主机上的相关docker容器服务 (后台守护态运行)，在脚本 `datum-network-deploy` 根目录执行：
...
```shell
# 在主控机上停止各个目标主机上的相关docker容器服务，在脚本 `datum-network-deploy` 根目录执行：
...
# 然后再在主控机上清除各个目标机上的相关docker容器的数据和安装的docker镜像，以及相关的配置等信息 (清除所有数据)，在脚本 `datum-network-deploy` 根目录执行：
